...
//    console.log(nodes.length);
    ...
    nodes.map(function(current, index, array) {
      ...
      var xpaths = {
        'url' : '//results/div[contains(@class,"database_entry")]['+bar+']/div/div/div[@class="database_entry_title"]/a/@href',
        'image' : '//results/div[contains(@class,"database_entry")]['+bar+']/div[@class="database_entry_image"]/a/img/@src',
      }
      
  //     var foo = xpath.select(myxpath, doc)[0].value;
      
      var foo;
      var qux;
      var entry = {};
      for (foo in xpaths) {
        var baz = xpath.select(xpaths[foo], doc)[0];
        if(baz.value){
          qux = baz.value;
        }
        else {
...
//  'commonNames' : '//div[@class="database_entry_property"]/span/text()[contains(.,"Common Name")]/../../text() | //div[@class="database_entry_property"]/span/text()[contains(.,"Common Name")]/../../descendant::*/text()',
//  'plantTypes' : '//div[@class="database_entry_property"]/span/text()[contains(.,"Plant Type")]/../../text()',
...
//var foo;
//for (foo in xpaths) {
//  output[foo] = xpath.select(xpaths[foo], doc)
//    .map(function(node) {
//      if(node.value){
//        return node.value;
//      }
